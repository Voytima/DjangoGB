...
class TestCoursesPage(TestCase):
    def test_page_open(self):
        path = reverse("mainapp:courses_list")
        result = self.client.get(path)
        self.assertEqual(result.status_code, HTTPStatus.OK)
...
class TestNewsPage(TestCase):
    ...
    def setUp(self):
        ...
        self.client_with_auth.post(
            path_auth, data={"username": "admin", "password": "qweqweqwe"}
        )
...
class TestCoursesWithMock(TestCase):
    ...
    "mainapp/fixtures/001_lessons.json",
    "mainapp/fixtures/001_courses.json",
    "mainapp/fixtures/001_teachers.json",
    )
...
class TestTaskMailSend(TestCase):
    fixtures = ("authapp/fixtures/001_user.json",)
...
class TestNewsSelenium(StaticLiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.selenium.find_element(By.ID, "id_password").send_keys("qweqweqwe")
        button_enter.click()
    ...
    def test_pick_color(self):
        ...
        try:
            self.assertEqual(
            navbar_el.value_of_css_property("background-color"), "rgb(255, 255, 155)",
        )
